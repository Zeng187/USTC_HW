...
void Attribute::ComponentVisitor::ImplVisit(Ptr<CmptSimulate> simulate) {
	...
	grid->AddButton("set y top fix", [simulate]() {
		simulate->SetTopFix();
	});

	grid->AddButton("set corners fix", [simulate]() {
		simulate->SetCornerFix();
	});
	grid->AddButton("AccInit", [simulate]() {
		simulate->AccInit();
	});
...
void Attribute::ComponentVisitor::ImplVisit(Ptr<TriMesh> mesh) {
	...
	grid->AddLine();
	std::vector<std::string> parameterizingWMethodStrs = { "Uniform", "Cotangent","Shape preserving" };
	Ptr<Paramaterize::Paramethod> pmethod = std::make_shared<Paramaterize::Paramethod>();
	*pmethod = Paramaterize::Paramethod::kUniform;
	grid->AddComboBox("Weight Method", parameterizingWMethodStrs, [pmethod](const std::string& str)
	{
		if (str == "Uniform")
			*pmethod = Paramaterize::Paramethod::kUniform;
		else if (str == "Cotangent")
			*pmethod = Paramaterize::Paramethod::kCot;
		else if (str == "Shape preserving")
			*pmethod = Paramaterize::Paramethod::kShapePreserving;

	});

	std::vector<std::string> parameterizingBMethodStrs = { "Round", "Square" };
	Ptr<Paramaterize::ParaBound> pBoundary = std::make_shared<Paramaterize::ParaBound>();
	*pBoundary = Paramaterize::ParaBound::kCircle;
	grid->AddComboBox("Boundary Method", parameterizingBMethodStrs, [pBoundary](const std::string& str)
	{
		if (str == "Round")
			*pBoundary = Paramaterize::ParaBound::kCircle;
		else if (str == "Square")
			*pBoundary = Paramaterize::ParaBound::kRect;
	});


	grid->AddButton("Parameterize", [mesh, pOGLW = attr->pOGLW, pmethod, pBoundary]() {
		auto paramaterize = Paramaterize::New(mesh);
		if (paramaterize->Run(*pmethod, *pBoundary))
			printf("Parameterize done\n");
		pOGLW->DirtyVAO(mesh);
	});


	grid->AddButton("Parameter Result", [mesh, pOGLW = attr->pOGLW, pmethod, pBoundary]() {
		auto paramaterize = Paramaterize::New(mesh);
		if (paramaterize->Run(*pmethod, *pBoundary))
			printf("Parameterize done\n");
		paramaterize->ShowParameterizeResult();
		pOGLW->DirtyVAO(mesh);
	});
	...
	grid->AddButton("ARAP", [mesh, pOGLW = attr->pOGLW]() {
		...
		if (arap->Run())
			printf("ARAP Parameterize done\n");
		pOGLW->DirtyVAO(mesh);
	});


	grid->AddButton("ARAP result", [mesh, pOGLW = attr->pOGLW]() {
		auto arap = ARAP::New(mesh);
		if (arap->Run())
			printf("ARAP Parameterize done\n");
		arap->ShowParameterizeResult();
		pOGLW->DirtyVAO(mesh);
	});
	...
	grid->AddButton("ASAP", [mesh, pOGLW = attr->pOGLW]() {
		...
		if (asap->Run())
			printf("ARSP Parameterize done\n");
		pOGLW->DirtyVAO(mesh);
	});


	grid->AddButton("ASAP result", [mesh, pOGLW = attr->pOGLW]() {
		auto asap = ASAP::New(mesh);
		if (asap->Run())
			printf("ASAP Parameterize done\n");
		asap->ShowParameterizeResult();
		pOGLW->DirtyVAO(mesh);
	});
